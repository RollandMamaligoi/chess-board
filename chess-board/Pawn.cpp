...
#include "Player.h"
#include <iostream>
std::vector<std::pair<int, int>> Pawn::getPossibleMoves(const Board& board, int posX, int posY)
{
    ...
    if (pieceColour == Colour::WHITE) {
        ...
        if (posY == 4) {
            std::string lastMove = board.lastMove(Colour::BLACK);
            int x = lastMove[1] - 'a';
            int y = lastMove[2] - '1';
            if (posX + 1 < 8) {
        ...
        if (posY == 3) {
            std::string lastMove = board.lastMove(Colour::WHITE);
            int x = lastMove[1] - 'a';
            int y = lastMove[2] - '1';
            if (posX + 1 < 8) {
                if (x == posX + 1 && y == posY && lastMove[0] == 'P') {
...
void Pawn::setFirstMove(bool moved)
{
    firstMove = moved;
}